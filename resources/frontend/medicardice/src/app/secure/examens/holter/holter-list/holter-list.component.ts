...
import {Holter} from "../../../../shared/models/welcome";
...
@Component({
  selector: 'app-holter-list',
  templateUrl: './holter-list.component.html',
  styleUrl: './holter-list.component.css'
})
export class HolterListComponent implements OnInit {

  userData: Holter[]=[];
  isExpired!:boolean;
  url=signal("/dashboard/holter/add");
  label=signal('nouvel holter');
  labelTitle=signal('');
  items!:Holter[];
  ...
  private getAll(){
    return this.holterService.getAll()
      .subscribe({
        next:res=>{
  ...
  deleteHolter(id: number) {
    ...
    swalWithTailwindButtons.fire({
      title: 'Êtes-vous sûr?',
      text: "De vouloir supprimer ce holter !",
      icon: 'warning',
    ...
    this.router.navigate(['/dashboard/holter/list'])
  }
}
