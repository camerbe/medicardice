...
class AppointmentRepository extends BaseRepository
{
    ...
    {
        if(auth()->user()->roles()->first()->role=='Doctor'){
            $patientName=$this->findById($id)->patient()->first()->created_by;
            $appointment=$this->findById($id)->first();
            $user_id=Patient::find($appointment->patient_id)->user_id;
            $patientEmail=User::find($user_id)->email;
            $date_rdv=Carbon::parse($appointment->appointment_date)->translatedFormat('d F Y');
            $date_heure=Carbon::parse($appointment->appointment_date)->format('H:i');
            Mail::to($patientEmail)->send(new AnnulationMail($patientName,$date_rdv,$date_heure));
        }
    ...
    public function update(array $input, $id)
    {
        ...
        $input['appointment_date']= Carbon::parse($input['appointment_date'])->format('Y-m-d H:i') ?? $currentAppointment->appointment_date;
